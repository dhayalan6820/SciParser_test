...
**Related pitfalls fixed in the same feature:**
- A "selected count" badge that sums two auto-derived lists (selected messages + their auto-derived tools) looks like a bug to users ("I selected 2 things but it says 5") even though the counting is technically correct — when tools are auto-derived from message selection, only show the message count in message-selection UI; show tool counts only inside the tools-specific panel.
- Any modal/dialog that performs its own async "create" action independently of the parent (e.g. calls its own API function rather than a parent-owned submit handler) needs an explicit success callback prop (e.g. `onScheduled`) so the parent can reset selection-mode/local state — a parent-level "on submit, reset state" handler that the child dialog never calls will silently leave stale selection UI active after success.
