  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <KPICard index={0} icon={<Gauge className="h-4 w-4" />} label="Average Latency" value={`${data.avg_latency}ms`} />
        <KPICard index={1} icon={<Gauge className="h-4 w-4 text-blue-500" />} label="Median Latency" value={`${data.median_latency}ms`} />
        <KPICard index={2} icon={<Gauge className="h-4 w-4 text-indigo-500" />} label="P90 Latency" value={`${data.p90_latency}ms`} />
        <KPICard index={3} icon={<Gauge className="h-4 w-4 text-purple-500" />} label="P95 Latency" value={`${data.p95_latency}ms`} />
        <KPICard index={4} icon={<Gauge className="h-4 w-4 text-red-500" />} label="P99 Latency" value={`${data.p99_latency}ms`} />
      </div>
        ...
        <div className="space-y-4 pt-1 text-xs">
          <div>
            <div className="flex justify-between text-[11px] font-semibold text-slate-500 mb-1">
              ...
              <span>{data.inference_time_avg}ms</span>
            </div>
          ...
          <div>
            <div className="flex justify-between text-[11px] font-semibold text-slate-500 mb-1">
              ...
              <span>{data.tool_time_avg}ms</span>
            </div>
            <div className="h-2 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
          ...
          <div>
            <div className="flex justify-between text-[11px] font-semibold text-slate-500 mb-1">
              ...
              <span>{data.memory_time_avg}ms</span>
            </div>
            <div className="h-2 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-muted-foreground pl-7">
                        {new Date(err.timestamp).toLocaleString()} • Conversation: <strong className="font-mono text-[9px]">{err.conversation_id}</strong>
                      </p>
                    </div>
