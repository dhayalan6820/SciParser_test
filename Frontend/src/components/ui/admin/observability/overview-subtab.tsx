  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard index={0} icon={<Activity className="h-4 w-4" />} label="Total Agent Runs" value={(data?.total_runs ?? 0).toLocaleString()} />
        <KPICard index={1} icon={<Cpu className="h-4 w-4" />} label="Total Tool Calls" value={(data?.total_tool_calls ?? 0).toLocaleString()} />
        <KPICard index={2} icon={<Coins className="h-4 w-4" />} label="Total API Cost" value={`$${(data?.total_cost ?? 0).toFixed(4)}`} />
        <KPICard index={3} icon={<Gauge className="h-4 w-4" />} label="Success Rate" value={`${data?.overall_success_rate ?? 0}%`} />
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
        <KPICard index={4} icon={<Layers className="h-4 w-4" />} label="Prompt Tokens" value={(data?.total_prompt_tokens ?? 0).toLocaleString()} />
        <KPICard index={5} icon={<Layers className="h-4 w-4 text-blue-500" />} label="Completion Tokens" value={(data?.total_completion_tokens ?? 0).toLocaleString()} />
        <KPICard index={6} icon={<Layers className="h-4 w-4 text-emerald-500" />} label="Cached Tokens Saved" value={(data?.total_cached_tokens ?? 0).toLocaleString()} />
        <KPICard index={7} icon={<Activity className="h-4 w-4 text-purple-500" />} label="Avg Response Time" value={`${data?.avg_latency_ms ?? 0}ms`} />
      </div>
